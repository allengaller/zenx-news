...
if (!class_exists('WumiiRelatedPosts')) {
    class WumiiRelatedPosts {
        ...
        private function getRelatedItemsHtml() {
            ...
            global $wp_version;
            $path = "/ext/relatedItemsRss?type=1&url=$encodedUrl&title=$encodedTitle&num=$num&sitePrefix=$encodedSitePrefix&mode=3&v=" . self::VERSION . "&pf=WordPress$wp_version";
...
WUMII_HOOK;
        ...
        private function createWumiiScript() {
            ...
            $jsonCategories = $this->getJsonCategories();
...
WUMII_SCRIPT;
        ...
        private function getJsonCategories() {
            $categories = get_the_category();
            $categoryArr = array();
            if ($categories) {
                foreach($categories as $category) {
                    $categoryArr[] = html_entity_decode($category->cat_name);
                }
            }
        ...
        function printAdminPage() {
            ...
            if (array_key_exists('add_verification_meta', $_POST)) {
                ...
                if ($this->isHookFuncInTemplate('header.php', 'wp_head')) {
                    ...
                    if ($code) {
                        $OptionsUpdatedMessage[] = new Wumii_Notify_Message('已成功添加认证代码，现在请回到<a href="http://www.wumii.com/site/index" target="_blank">无觅网站管理中心</a>，点击“验证”按钮即可完成博客认证。');
                    }
                } else {
        ...
        function doActivation() {
            ...
            $this->pushNotifyMessage('<strong>无觅相关文章插件: </strong>如果您的博客装有缓存(cache)插件, 请在首次启用插件时清空缓存. 另外, 启用插件后无觅爬虫会立刻去获取贵站的文章信息, 这时切记不要停掉插件, 以免影响文章的相关性.装完可来<strong><a href="http://www.wumii.com/site/index" target="_blank">无觅网站管理中心</a></strong>查看收录状况.');
        }
