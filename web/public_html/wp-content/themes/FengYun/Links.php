...
<?php get_header(); ?>
	...
	<div class="entry_box_s_l">
		<div class="link_page">
			<h2>申明：如您的链接没有在首页出现，表着急，刷新一下就有了，因为本站链接是不分先后随机排序的。为了页脚美观和控制PR输出，首页只放32个链接，莫怪！</h2>
			<?php
				if(function_exists(’wp_dtree_get_links’)){
				wp_dtree_get_links();
				}else{
				my_list_bookmarks('title_li=&categorize=1&category=2&orderby=rand&show_images=1');
				}
			?>
			<div class="clear"></div>
		</div>
