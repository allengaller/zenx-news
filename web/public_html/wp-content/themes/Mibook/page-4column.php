                        ...
                        <li class="<?php echo $thumbClass; ?>">
                        	<div class="port4_img foldify <?php if ( $titles ) echo ( ' title_off' ); ?>">
								<?php if (has_post_thumbnail()){
									...
									<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>" target="_blank"><img src="<?php echo $image_src[0]; ?>" alt="<?php the_title(); ?>" width="200px" height="140px" /></a> 
								<?php }else {?>
									<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title(); ?>" target="_blank"><img src="<?php echo catch_post_image();?>" alt="<?php the_title(); ?>" width="200px" height="140px" /></a>
								<?php } ?>
                            </div>
							<?php if ( !$titles ) { ?>
                        	<div class="port_caption">
                            	<a class="port_title" href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf( esc_attr__( '%s', 'mibook' ), the_title_attribute( 'echo=0' ) ); ?>" target="_blank"><?php the_title(); ?></a></div> <?php } ?>
						</li>
                        <?php $counter++; 
                        if($counter > $col){
                        $col = $col + 4; ?>
